...
class MiningRoutes {
  ...
  private async $getHistoricalPrice(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (['testnet', 'signet', 'regtest', 'liquidtestnet'].includes(config.MEMPOOL.NETWORK)) {
  ...
  private async $getAccelerationsByPool(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.header('Cache-control', 'public');
      res.setHeader('Expires', new Date(Date.now() + 1000 * 60).toUTCString());
      if (!config.MEMPOOL_SERVICES.ACCELERATIONS || ['testnet', 'signet', 'regtest', 'liquidtestnet', 'liquid'].includes(config.MEMPOOL.NETWORK)) {
